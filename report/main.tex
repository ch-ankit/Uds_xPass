...
carry approximately 20--25\% lower completion odds after controlling for
pass difficulty, an effect that is significant in 14 of the 17 modern seasons (1973/74 excluded, n~=~469, wrong-signed). The
...
ECE~=~0.003), but calibration degrades by up to a factor of 19 on rare
pass types (Through-ball ECE~=~0.058). A feature ablation shows that
footedness features improve calibration only marginally and that additional
foot-by-type interactions overfit, and inverse-frequency reweighting of
rare types degrades calibration almost everywhere. Per-type isotonic recalibration halves
the Through-ball ECE while slightly worsening Cross and Switch, succeeding
exactly where samples suffice and failing where they are thin. We conclude
that poor calibration on rare pass types is driven primarily by data
scarcity rather than by missing footedness features, and that adding
...
far larger (246\,740 usable passes) and 2020/21 is a full season. Raw
events are cached to data/passes-\textless
season-id\textgreater.parquet (or .pkl where pyarrow is
...
per-season logit controls. Second, 1973/74 contributes only 469 passes
...
(preferred-foot-fifa.csv, fuzzy name matching; raw dump
players-21.csv). The broader source search across FBref,
Transfermarkt, FIFA, and Wikidata is frozen in archive/ for
...
only in footedness features --- no-footedness (weak flag and
wf$\times$cross removed), current (weak flag
plus wf$\times$cross), and full-interactions
(adding wf$\times$switch and
wf$\times$through-ball) --- and compare Brier/ECE per
pass type on pooled held-out predictions. A follow-up weighting experiment
(weighted-study.py) trains the same model with sample weights
emphasising rare types --- unweighted ($w=1$),
inv-type ($w \propto 1/\mathrm{frequency}$), and a milder
sqrt-inv-type --- on the five largest cached seasons. For recalibration we fit
...
negative in every modern season, ranging from about $-0.10$ to $-0.43$,
corresponding to roughly 20--25\% lower completion odds at fixed
difficulty, and is statistically significant in 14 of the 17 modern seasons (1973/74 excluded). The
2015/16 estimate is the most precise ($-0.254$, $p < 10^{-60}$,
n~=~246\,740). Raw gaps are smaller (typically 0--4 percentage points)
...
weak-foot-hypothesis-by-season.csv.
\begin{figure}[h]
\centering
...
(calibration-summary-by-season.csv), confirming the model is
...
Figure~\ref{fig:cal}). Ordinary passes (n~=~142\,420) are near-perfectly
calibrated (ECE~=~0.003, Brier~=~0.091). Calibration then degrades
monotonically with scarcity: Switch (n~=~3\,741, ECE~=~0.017), Cross
(n~=~1\,030, ECE~=~0.044), Through ball (n~=~1\,107, ECE~=~0.058,
Brier~=~0.236). The model is systematically overconfident on every rare
...
\midrule
Ordinary & 142420 & 0.824 & 0.824 & 0.091 & 0.003 \\
Switch & 3741 & 0.785 & 0.791 & 0.122 & 0.017 \\
Cross & 1030 & 0.260 & 0.269 & 0.185 & 0.044 \\
Through ball & 1107 & 0.392 & 0.400 & 0.236 & 0.058 \\
\bottomrule
\end{tabular}
\caption{Calibration by pass type, pooled across seasons
(calibration-summary-by-pass-type.csv). Through balls are
$19\times$ worse calibrated than Ordinary passes.}
\label{tab:cal}
\end{table}
\begin{figure}[h]
\centering
...
\subsection{Finding 3: ablation --- footedness helps marginally, more of it overfits}
\label{sec:ablation}
Figure~\ref{fig:abl} compares the three footedness variants. The current
feature set marginally beats no-footedness on the hardest
types --- Cross ECE 0.044 versus 0.047, Through-ball 0.058 versus 0.063
--- while Ordinary passes are identical (ECE~=~0.003) everywhere.
full-interactions wins only on Switch (ECE 0.015 versus 0.017)
and overfits elsewhere (Cross 0.049, Through 0.063). In other words, the
model can use a blanket weak-foot penalty, but giving it a separate
penalty per type spreads roughly 5\,000 rare-type examples across too
many parameters. Missing footedness features are not the cause of the
calibration gap.
\begin{figure}[h]
\centering
...
Inverse-frequency weighting degrades ECE on Ordinary (0.003 to 0.007),
Cross (0.056 to 0.064), and especially Through balls (0.045 to 0.114),
while overall discrimination drops slightly; only Switch improves
slightly (ECE 0.031 to 0.026, Brier 0.117 to 0.113), and the milder
...
under imbalance, but it works against calibration by construction --- a
...
Type & n & \multicolumn{2}{c}{Unweighted} & \multicolumn{2}{c}{Inv.\ weighted} \\
 & & Brier & ECE & Brier & ECE \\
\midrule
Ordinary & 81997 & 0.095 & 0.003 & 0.099 & 0.007 \\
Switch & 2179 & 0.117 & 0.031 & 0.113 & 0.026 \\
Cross & 666 & 0.176 & 0.056 & 0.188 & 0.064 \\
Through ball & 539 & 0.233 & 0.045 & 0.254 & 0.114 \\
\bottomrule
\end{tabular}
\caption{Type-weighted training versus unweighted, pooled held-out
predictions over the five largest cached seasons
(weighted-summary.csv). Weighting helps only Switch, marginally,
and harms everything else.}
\label{tab:weight}
\end{table}

\subsection{Finding 4: recalibration succeeds exactly where data suffices}
...
here. Figure~\ref{fig:recal} shows before/after reliability per type. On
Through balls (evaluation n~=~554) ECE falls from 0.058 to 0.029 and
Brier from 0.239 to 0.231 --- the correction roughly halves the error.
On Cross (n~=~515) and Switch (n~=~1\,871) both metrics worsen slightly
(Cross ECE 0.061 to 0.062; Switch 0.020 to 0.028), and Ordinary
(n~=~71\,210) is unchanged (ECE~=~0.003). The correction succeeds where
volume is sufficient and fails where data is thin --- the exact signature
of sample scarcity as the root cause.
\begin{figure}[h]
\centering
...
The all-seasons.py 360 flag re-runs the notebook's
360-enriched comparison for the one season with freeze-frame data (code path available; figure omitted from poster-outputs). Adding
...
itself by sample size across types. Third, neither richer footedness
...
Fourth, a flexible recalibration fixes
the best-sampled rare type and harms the thinner ones. Together they imply
...
(a $\sim$20\% odds adjustment is material for high-volume crossers of
...
and match time, and possession-sequence context are unmodelled. Early
...
python all-seasons.py (full multi-season run; --seasons
flags allow quick tests; --with-360 adds the bonus) followed by
python calibration-study.py regenerates every figure and CSV in
poster-outputs/, and python weighted-study.py
regenerates the weighting comparison (weighted-summary.csv);
see the README for dependencies. The frozen
analysis notebook, foot-source scrapers, the xPass.ipynb scratch
check behind the 1973/74 exclusion, and the early single-season figures
and dashboard are preserved in archive/.
