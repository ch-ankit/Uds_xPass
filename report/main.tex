...
ECE~=~0.003), but calibration degrades by up to a factor of 14 on rare
pass types (Through-ball ECE~=~0.043). A feature ablation shows that
footedness features improve calibration only marginally and inconsistently across types, and inverse-frequency reweighting of
rare types degrades calibration almost everywhere. Per-type isotonic recalibration fails to improve any rare type --- worsening Cross, Switch, and Through-ball ECE alike while leaving Ordinary untouched --- which itself confirms the diagnosis: with only hundreds to a few thousand examples per rare type, even the correction overfits. We conclude
...
(4) recalibration and reweighting experiments whose uniform failure on rare types
diagnoses scarcity as the root cause.
...
far larger (307\,973 usable passes) and 2020/21 is a full season. Coordinates follow the StatsBomb 120$\times$80 grid (verified on cache: start x spans 0.1--120.9, y 0.1--80.8), with the goal centre at (120, 40); all distances, angles, and pitch zones are computed on this grid. Raw
...
pass-cut-back is nearly absent in this pull (pooled n~=~145, ECE~=~0.070): it is shown in the calibration table for completeness but excluded from the ablation and recalibration as too thin to split, and dropped from the
per-season logit controls wherever it is zero-variance. Second, 1973/74 contributes only 555 passes
with incomplete body-part tagging and a wrong-signed, non-significant logit coefficient (p~=~0.12); it is
...
(preferred_foot_fifa.csv, fuzzy name matching; raw dump
players_21.csv). The broader source search across FBref,
...
(weighted_study.py) trains the same model with sample weights
...
weak_foot_hypothesis_by_season.csv.
\begin{figure}[h]
\centering
...
(calibration_summary_by_season.csv), confirming the model is
...
(calibration_summary_by_pass_type.csv). Through balls are
$14\times$ worse calibrated than Ordinary passes.}
\label{tab:cal}
\end{table}
\begin{figure}[h]
\centering
...
(weighted_summary.csv). Weighting helps only Switch, marginally,
...
The all_seasons.py 360 flag re-runs the notebook's
360-enriched comparison for the one season with freeze-frame data (code path available; figure omitted from poster_outputs). Adding
...
python all_seasons.py (full multi-season run; --seasons
flags allow quick tests; --with-360 adds the bonus) followed by
python calibration_study.py regenerates every figure and CSV in
poster_outputs/, and python weighted_study.py
regenerates the weighting comparison (weighted_summary.csv);
